...
for x in csv.DictReader(open(str(PROJECT_ROOT / 'cargen_report_links.csv'),encoding='utf-8')):
 title=x['title'];url=x['url'];low=(title+' '+x['filename']).lower();ys=re.findall(r'20\d{2}',title+' '+x['filename']);year=ys[0] if ys else ''
 if not year:continue
 if any(k in low for k in ['half','interim','march','june','september','sep']):freq='Semi-annual / half-year' if ('half' in low or 'june' in low) else 'Quarterly'
 else:freq='Annual / full-year'
 subtype='Annual report' if 'annual report' in low or 'integrated report' in low or 'financial report' in low else 'Financial results / statements'
 if 'interim' in low or 'half' in low or 'unaudited' in low:subtype='Financial results / statements'
 key=('Car & General (Kenya) PLC',url)
 if key in existing:continue
 readable=title.replace('_',' ').replace('-',' ').replace('.pdf','').strip()
 rows.append({'record_id':'','issuer':'Car & General (Kenya) PLC','ticker':'CGEN','report_frequency':freq,'document_subtype':subtype,'report_year_label':year,'webpage_title':readable,'source_page_url':'https://cargen.com/financial-reports/','download_url':url,'http_status':'linked from issuer page','content_type':'application/pdf (inferred from .pdf URL)','source_tier':'Issuer website'});existing.add(key);new+=1
