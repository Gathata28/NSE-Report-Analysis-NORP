...
for x in csv.DictReader(open(str(PROJECT_ROOT / 'kakuzi_report_links.csv'),encoding='utf-8')):
 title=x['title'];url=x['url'];low=title.lower()
 if 'interim' in low or 'june' in low:freq='Semi-annual / half-year'
 else:freq='Annual / full-year'
 subtype='Results booklet / abridged report' if 'extract from' in low else ('Financial results / statements' if 'financial statement' in low else 'Annual report')
 ys=re.findall(r'20\d{2}',title)
 year=ys[-1] if ys else ''
 if not year:continue
 key=('Kakuzi PLC',url)
 if key in existing:continue
 rows.append({'record_id':'','issuer':'Kakuzi PLC','ticker':'KUKZ','report_frequency':freq,'document_subtype':subtype,'report_year_label':year,'webpage_title':title,'source_page_url':'https://www.kakuzi.co.ke/company-reports','download_url':url,'http_status':'linked from issuer page','content_type':'application/pdf (inferred from .pdf URL)','source_tier':'Issuer website'});existing.add(key);new+=1
