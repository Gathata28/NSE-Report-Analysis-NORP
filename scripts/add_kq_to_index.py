...
for x in csv.DictReader(open(str(PROJECT_ROOT / 'kq_annual_report_links.csv'),encoding='utf-8')):
 title=x['title'];url=x['url'];low=(title+' '+url).lower()
 if 'half' in low or 'six month' in low:freq='Semi-annual / half-year'
 elif 'quarter' in low:freq='Quarterly'
 else:freq='Annual / full-year'
 yearmatch=re.findall(r'20\d{2}',title+' '+url)
 if 'fy24' in low:year='2024'
 elif 'fy23' in low:year='2023'
 elif '2021' in low:year='2021'
 elif '2020' in low:year='2020'
 elif yearmatch:year=yearmatch[0]
 else:continue
 key=('Kenya Airways PLC',url)
 if key in existing:continue
 rows.append({'record_id':'','issuer':'Kenya Airways PLC','ticker':'KQ','report_frequency':freq,'document_subtype':'Annual report','report_year_label':year,'webpage_title':title,'source_page_url':'https://corporate.kenya-airways.com/en/investors-shareholders/annual-reports/','download_url':url,'http_status':'linked from issuer page','content_type':'application/pdf (inferred from .pdf URL)','source_tier':'Issuer website'});existing.add(key);new+=1
