...
for x in csv.DictReader(open(str(PROJECT_ROOT / 'scb_report_links.csv'),encoding='utf-8')):
    title=x['title'];url=x['url'];issuer='Standard Chartered Bank Kenya Ltd';key=(issuer,url)
    if key in seen:continue
    low=title.lower()
    if 'annual report' in low or 'annual financial' in low:freq='Annual / full-year'
    elif re.search(r'(h1|half|hy)',low):freq='Semi-annual / half-year'
    elif re.search(r'(q1|q2|q3|q4|quarter)',low):freq='Quarterly'
    elif 'full year' in low or 'audited result' in low:freq='Annual / full-year'
    else:freq='Periodic results material'
    if 'annual report' in low:sub='Annual report'
    elif 'financial statement' in low or 'financials' in low or 'audited results' in low or 'unaudited results' in low:sub='Financial results / statements'
    ...
    rows.append({'record_id':'','issuer':issuer,'ticker':'SCBK','report_frequency':freq,'document_subtype':sub,'report_year_label':year,'webpage_title':'Investor Relations – Standard Chartered Kenya','source_page_url':'https://www.sc.com/ke/investor-relations/','download_url':url,'http_status':'linked from issuer page','content_type':'application/pdf (inferred from .pdf URL)','source_tier':'Issuer website'});seen.add(key);new+=1
