...
for x in csv.DictReader(open(str(PROJECT_ROOT / 'dtb_quarterly_links.csv'),encoding='utf-8')):
 title=x['title'];url=x['url'];issuer='Diamond Trust Bank Kenya Ltd';key=(issuer,url)
 if key in seen:continue
 low=title.lower();freq='Annual / full-year' if 'full year' in low or 'fourth quarter' in low else ('Semi-annual / half-year' if ('half' in low or 'second quarter' in low) else 'Quarterly')
 sub='Financial results / statements'
 m=re.search(r'(20\d{2})',title);year=m.group(1) if m else ''
 rows.append({'record_id':'','issuer':issuer,'ticker':'DTK','report_frequency':freq,'document_subtype':sub,'report_year_label':year,'webpage_title':'Diamond Trust Bank Kenya','source_page_url':'https://dtbk.dtbafrica.com/quarterly-financial-reports','download_url':url,'http_status':'linked from issuer page','content_type':'application/pdf (inferred from .pdf URL)','source_tier':'Issuer website'});seen.add(key);new+=1
