...
for x in csv.DictReader(open(str(PROJECT_ROOT / 'eabl_financial_results_links.csv'),encoding='utf-8')):
    ...
    key=('East African Breweries PLC',url)
    if key in seen:continue
    low=title.lower()
    freq='Annual / full-year' if 'full year' in low else 'Semi-annual / half-year'
    subtype='Financial results / statements' if 'financials' in low or 'financial results' in low else ('Investor presentation' if 'presentation' in low else ('Results announcement' if 'press' in low else 'Periodic results material'))
    m=re.search(r'(20\d{2})',title);year=m.group(1) if m else ''
    rows.append({'record_id':'','issuer':'East African Breweries PLC','ticker':'EABL','report_frequency':freq,'document_subtype':subtype,'report_year_label':year,'webpage_title':'Financial Results | EABL','source_page_url':'https://www.eabl.com/investors/financial-results','download_url':url,'http_status':'linked from issuer page','content_type':'application/pdf (inferred from .pdf URL)','source_tier':'Issuer website'})
    seen.add(key);new+=1
