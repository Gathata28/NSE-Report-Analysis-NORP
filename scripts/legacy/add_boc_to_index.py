...
for x in csv.DictReader(open(str(PROJECT_ROOT / 'boc_report_links.csv'),encoding='utf-8')):
 title=x['title'];url=x['url'];low=title.lower();ys=re.findall(r'20\d{2}',title)
 if not ys:ys=re.findall(r'20\d{2}',x['filename'])
 year=ys[-1] if ys else ''
 if not year:continue
 key=('BOC Kenya PLC',url)
 if key in existing:continue
 rows.append({'record_id':'','issuer':'BOC Kenya PLC','ticker':'BOC','report_frequency':'Annual / full-year','document_subtype':'Annual report','report_year_label':year,'webpage_title':title,'source_page_url':'https://www.boc.co.ke/our-business/investor-relations','download_url':url,'http_status':'linked from issuer page','content_type':'application/pdf (official Linde asset endpoint)','source_tier':'Issuer website'});existing.add(key);new+=1
