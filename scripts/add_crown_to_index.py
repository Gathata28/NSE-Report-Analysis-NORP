...
for x in csv.DictReader(open(str(PROJECT_ROOT / 'crown_report_links.csv'),encoding='utf-8')):
 title=x['title'];url=x['url'];low=title.lower()
 if 'half year' in low or 'hy2026' in low or 'six months' in low:freq='Semi-annual / half-year'
 elif 'quarter' in low:freq='Quarterly'
 else:freq='Annual / full-year'
 subtype='Annual report' if 'annual report' in low else 'Financial results / statements'
 ys=re.findall(r'20\d{2}',title+' '+url);year=ys[0] if ys else ''
 if not year:continue
 key=('Crown Paints Kenya PLC',url)
 if key in existing:continue
 rows.append({'record_id':'','issuer':'Crown Paints Kenya PLC','ticker':'CRWN','report_frequency':freq,'document_subtype':subtype,'report_year_label':year,'webpage_title':title,'source_page_url':'https://www.crownpaints.co.ke/about?tab=report#report','download_url':url,'http_status':'linked from issuer page','content_type':'application/pdf (inferred from .pdf URL)','source_tier':'Issuer website'});existing.add(key);new+=1
