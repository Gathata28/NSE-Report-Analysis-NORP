...
for x in csv.DictReader(open(str(PROJECT_ROOT / 'bat_report_links.csv'),encoding='utf-8')):
 title=x['title'];url=x['url'];low=(title+' '+url).lower()
 if 'half year' in low or 'half-year' in low:freq='Semi-annual / half-year'
 elif 'quarter' in low:freq='Quarterly'
 else:freq='Annual / full-year'
 subtype=x['kind']
 ys=re.findall(r'20\d{2}',title+' '+url);year=ys[0] if ys else ''
 if not year:continue
 key=('British American Tobacco Kenya PLC',url)
 if key in existing:continue
 rows.append({'record_id':'','issuer':'British American Tobacco Kenya PLC','ticker':'BAT','report_frequency':freq,'document_subtype':subtype,'report_year_label':year,'webpage_title':title,'source_page_url':'https://www.batkenya.com/investors-and-reporting/financial-sustainability-reports','download_url':url,'http_status':'linked from issuer page','content_type':'application/pdf (inferred from .pdf URL)','source_tier':'Issuer website'});existing.add(key);new+=1
