...
The checked-in SQLite database is approximately 70 MB, below GitHub’s 100 MB hard limit but above its 50 MB recommendation. The current release keeps it available for immediate offline analysis and avoids adding Git LFS or an external artifact dependency. Future growth should move large SQLite snapshots, ZIP bundles, and duplicate PDFs to GitHub Releases, an external object store, or Git LFS after selecting an artifact-retention policy. New large binaries should not be added casually to normal Git history.
